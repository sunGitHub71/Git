/****************************************************************************
** Meta object code from reading C++ file 'openurl.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.6)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../openurl.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'openurl.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.6. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_openUrl[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
       9,    8,    8,    8, 0x05,

 // slots: signature, parameters, type, tag, flags
      23,    8,    8,    8, 0x08,
      37,    8,    8,    8, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_openUrl[] = {
    "openUrl\0\0signalsTest()\0onOkClicked()\0"
    "onCancelClicked()\0"
};

void openUrl::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        openUrl *_t = static_cast<openUrl *>(_o);
        switch (_id) {
        case 0: _t->signalsTest(); break;
        case 1: _t->onOkClicked(); break;
        case 2: _t->onCancelClicked(); break;
        default: ;
        }
    }
    Q_UNUSED(_a);
}

const QMetaObjectExtraData openUrl::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject openUrl::staticMetaObject = {
    { &BaseWindow::staticMetaObject, qt_meta_stringdata_openUrl,
      qt_meta_data_openUrl, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &openUrl::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *openUrl::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *openUrl::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_openUrl))
        return static_cast<void*>(const_cast< openUrl*>(this));
    return BaseWindow::qt_metacast(_clname);
}

int openUrl::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = BaseWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void openUrl::signalsTest()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}
QT_END_MOC_NAMESPACE
